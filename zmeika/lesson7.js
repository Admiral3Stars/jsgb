...
function haveFood(unit) {
    ...
    }else if(unit_classes.includes('boom-unit')){
        finishTheGame();
    }
    return check;
}
 ...
 */
function createBoom() {
    var boomCreated = false;

    while (!boomCreated) { //пока еду не создали
        // рандом
        var boom_x = Math.floor(Math.random() * FIELD_SIZE_X);
        var boom_y = Math.floor(Math.random() * FIELD_SIZE_Y);

        var boom_cell = document.getElementsByClassName('cell-' + boom_y + '-' + boom_x)[0];
        var boom_cell_classes = boom_cell.getAttribute('class').split(' ');

        // проверка на змейку
        if (!boom_cell_classes.includes('snake-unit')) {
            var classes = '';
            for (var i = 0; i < boom_cell_classes.length; i++) {
                classes += boom_cell_classes[i] + ' ';
            }

            boom_cell.setAttribute('class', classes + 'boom-unit');
            boomCreated = true;
        }
    }
}
